# Vendored from Ultralytics 8.3.239 (AGPL-3.0) — numpy-only helpers needed by
# the ByteTrack port. Sources: ultralytics/utils/ops.py, ultralytics/utils/metrics.py
import numpy as np


def xywh2ltwh(x: np.ndarray) -> np.ndarray:
    """Convert bounding box from (center x, center y, w, h) to (top-left x, top-left y, w, h)."""
    y = np.copy(x)
    y[..., 0] = x[..., 0] - x[..., 2] / 2
    y[..., 1] = x[..., 1] - x[..., 3] / 2
    return y
