...
# Vendored from Ultralytics 8.3.239 — batch_probiou (OBB-only) removed, bbox_ioa localized.

import numpy as np
import scipy
from scipy.spatial.distance import cdist

from .ops import bbox_ioa

try:
    import lap  # for linear_assignment (pip package: lapx)

    assert lap.__version__  # verify package is not directory
except (ImportError, AssertionError, AttributeError):
    lap = None  # falls back to scipy.optimize.linear_sum_assignment
...
def iou_distance(atracks: list, btracks: list) -> np.ndarray:
    ...
    if len(atlbrs) and len(btlbrs):
        # OBB (5-value probiou) branch removed in this vendored copy — axis-aligned boxes only
        ious = bbox_ioa(
